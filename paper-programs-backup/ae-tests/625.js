// Dashed Line Color Picker
// Keywords: markers, callbacks, function
// ------------------------------- //
// Required Programs (dependencies) Line Model, Line Drawing
...
// look for lineColorProperty
  const onProgramAdded = ( paperProgramNumber, scratchpad, sharedData ) => {
    ...
    if ( model.has( 'dashedlineColorProperty' ) ) {
      scratchpad.lineColor = model.get( 'dashedlineColorProperty' );
...
// 05.10.23 - use the y value as the marker position.  The y value is parameterized relative to the top and bottom of the rectangle.
  const onProgramMarkersChangedPosition = ( programNumber, programPoints, scratchpad, sharedData, currentMarkers ) => {
   ...
   //}
    scratchpad.dashedlineColor = 'rgb('+ scratchpad.redValue + ','+ scratchpad.greenValue +','+ scratchpad.blueValue +')';
  ...
  if ( model.has( 'dashedlineColorProperty' ) ) {
      ...
      phet.paperLand.getModelComponent( 'dashedlineColorProperty' ).value = scratchpad.dashedlineColor;
