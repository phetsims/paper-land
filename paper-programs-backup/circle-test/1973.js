...
(async () => {
  ...
  const onProgramAdded = ( paperProgramNumber, scratchpad, sharedData ) => {
    phet.paperLand.addModelComponent( 'pointBProperty', new phet.axon.Property( new phet.dot.Vector2( 0, 0 ) ) );
  };
  ...
  const onProgramChangedPosition = ( paperProgramNumber, positionPoints, scratchPad, sharedData ) => {
    ...
    if ( model.has( 'pointBProperty' ) ) {
      model.get( 'pointBProperty' ).value = displayCenter;
    }
  };

  // Called when the program is changed or no longer detected.
  const onProgramRemoved = ( paperProgramNumber, scratchpad, sharedData ) => {
      phet.paperLand.removeModelComponent( 'pointBProperty' );
  };
