...
(async () => {
  ...
  if (Math.abs(bobPosition.y - lowestPoint) <= range) { // Check if bob is within the range of the lowest point
    ...
    play();
      ...
      const bobCircleShape = phet.kite.Shape.circle( 50 )
      ...
      const setCenterX = ( x ) => {
        bobCirclePath.centerX = x;
        bobCirclePath.centerX = x;
      };
      
      const setCenterY = ( y ) => {
        bobCirclePath.centerY = y;
      };
      
      const setScale = ( scale ) => {
        bobCirclePath.scaleMagnitude = scale;
      };
...
const setLength = newValue => {
  ...
  ctx.fillText('1870', canvas.width / 2, canvas.height / 2 - 10);
