// Rectangle Drawing
// with marker-slider controls
// Keywords: simple, view, background, color
// =============================== //
// Program Dependencies: N/A
// Recommended Programs: n/a
// Program Description: defining a rectangle by an upper left to lower right vertices
...
(async () => {
  ...
  const onProgramAdded = ( paperProgramNumber, scratchpad, sharedData ) => {
    ...
    scratchpad.myRectangle = new phet.scenery.Rectangle( 10, 10, 400, 200, {
  ...
  const onProgramRemoved = ( paperProgramNumber, scratchpad, sharedData ) => {
    ...
    if ( scratchpad.myRectangle ){
      sharedData.scene.removeChild( scratchpad.myRectangle );
      scratchpad.myRectangle = null;
    }
  ...
  const onProgramMarkersChangedPosition = ( programNumber, programPoints, scratchpad, sharedData, currentMarkers ) => {
    ...
    if (firstMarker){
      const firstMarkerValue = (1-firstMarker.positionOnPaper.y);
    ...
    scratchpad.circleRadius = sharedData.displaySize.height/2*firstMarkerValue;
    phet.paperLand.console.log('new radius is', scratchpad.circleRadius);
