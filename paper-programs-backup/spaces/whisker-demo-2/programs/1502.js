...
(async () => {
  ...
  const onProgramAdjacent = ( programNumber, otherProgramNumber, direction, scratchpad, sharedData ) => {
    ...
    debugger;
    const myData = phet.paperLand.getProgramData( otherProgramNumber, 'myData' );
    if ( myData ) {

      // the other program has the data we care about! Do something with it.
      phet.paperLand.console.log( `I received: ${myData.value}!` );
    }
  };
